# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module GustoEmbedded
  module Models
    module Shared
      # Request body for updating a signatory. Email cannot be updated.
      class SignatoryUpdateRequest
        extend T::Sig
        include Crystalline::MetadataFields

        # Current version of the signatory (required for optimistic concurrency).
        field :version, ::String, { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('version'), required: true } }

        field :first_name, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('first_name') } }

        field :middle_initial, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('middle_initial') } }

        field :last_name, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('last_name') } }

        field :title, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('title') } }

        field :phone, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('phone') } }

        field :birthday, Crystalline::Nilable.new(::Date), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('birthday'), 'decoder': ::GustoEmbedded::Utils.date_from_iso_format(true) } }
        # The signatory's SSN.
        field :ssn, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('ssn') } }

        field :home_address, Crystalline::Nilable.new(Models::Shared::SignatoryUpdateRequestHomeAddress), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('home_address') } }

        sig { params(version: ::String, first_name: T.nilable(::String), middle_initial: T.nilable(::String), last_name: T.nilable(::String), title: T.nilable(::String), phone: T.nilable(::String), birthday: T.nilable(::Date), ssn: T.nilable(::String), home_address: T.nilable(Models::Shared::SignatoryUpdateRequestHomeAddress)).void }
        def initialize(version:, first_name: nil, middle_initial: nil, last_name: nil, title: nil, phone: nil, birthday: nil, ssn: nil, home_address: nil)
          @version = version
          @first_name = first_name
          @middle_initial = middle_initial
          @last_name = last_name
          @title = title
          @phone = phone
          @birthday = birthday
          @ssn = ssn
          @home_address = home_address
        end

        sig { params(other: T.untyped).returns(T::Boolean) }
        def ==(other)
          return false unless other.is_a? self.class
          return false unless @version == other.version
          return false unless @first_name == other.first_name
          return false unless @middle_initial == other.middle_initial
          return false unless @last_name == other.last_name
          return false unless @title == other.title
          return false unless @phone == other.phone
          return false unless @birthday == other.birthday
          return false unless @ssn == other.ssn
          return false unless @home_address == other.home_address
          true
        end
      end
    end
  end
end

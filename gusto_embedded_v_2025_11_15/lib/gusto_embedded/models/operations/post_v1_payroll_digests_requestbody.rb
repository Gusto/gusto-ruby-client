# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true

module GustoEmbedded
  module Models
    module Operations

      class PostV1PayrollDigestsRequestBody
        extend T::Sig
        include Crystalline::MetadataFields

        # A partner-generated unique identifier to ensure idempotency of the batch request. Scoped per partner.
        field(
          :idempotency_key,
          ::String,
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("idempotency_key"), required: true}}
        )
        # The action to perform on the batch.
        field(
          :batch_action,
          Models::Operations::PostV1PayrollDigestsBatchAction,
          {
            'format_json': {
              'letter_case': ::GustoEmbedded::Utils.field_name("batch_action"),
              required: true,
              'decoder': ::GustoEmbedded::Utils.enum_from_string(
                Models::Operations::PostV1PayrollDigestsBatchAction,
                false
              )
            }
          }
        )
        # Array of companies to fetch payroll digest data for. Maximum 25 companies per request.
        field(
          :batch,
          Crystalline::Array.new(Models::Operations::PostV1PayrollDigestsBatch),
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("batch"), required: true}}
        )

        sig {
          params(
            idempotency_key: ::String,
            batch_action: Models::Operations::PostV1PayrollDigestsBatchAction,
            batch: T::Array[Models::Operations::PostV1PayrollDigestsBatch]
          )
            .void
        }
        def initialize(idempotency_key:, batch_action:, batch:)
          @idempotency_key = idempotency_key
          @batch_action = batch_action
          @batch = batch
        end

        sig { params(other: T.untyped).returns(T::Boolean) }
        def ==(other)
          return false unless other.is_a?(self.class)
          return false unless @idempotency_key == other.idempotency_key
          return false unless @batch_action == other.batch_action
          return false unless @batch == other.batch
          true
        end
      end
    end
  end
end

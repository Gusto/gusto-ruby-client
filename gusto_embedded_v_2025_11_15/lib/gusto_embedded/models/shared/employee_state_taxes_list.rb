# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true

module GustoEmbedded
  module Models
    module Shared

      class EmployeeStateTaxesList
        extend T::Sig
        include Crystalline::MetadataFields

        # The employee's uuid
        field(
          :employee_uuid,
          Crystalline::Nilable.new(::String),
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("employee_uuid")}}
        )
        # Two letter US state abbreviation
        field(
          :state,
          Crystalline::Nilable.new(::String),
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("state")}}
        )

        field(
          :is_work_state,
          Crystalline::Nilable.new(Crystalline::Boolean.new),
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("is_work_state")}}
        )

        field(
          :questions,
          Crystalline::Nilable.new(Crystalline::Array.new(Models::Shared::EmployeeStateTaxQuestion)),
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("questions")}}
        )

        field(
          :file_new_hire_report,
          Crystalline::Nilable.new(Crystalline::Boolean.new),
          {'format_json': {'letter_case': ::GustoEmbedded::Utils.field_name("file_new_hire_report")}}
        )

        sig {
          params(
            employee_uuid: T.nilable(::String),
            state: T.nilable(::String),
            is_work_state: T.nilable(T::Boolean),
            questions: T.nilable(T::Array[Models::Shared::EmployeeStateTaxQuestion]),
            file_new_hire_report: T.nilable(T::Boolean)
          )
            .void
        }
        def initialize(employee_uuid: nil, state: nil, is_work_state: nil, questions: nil, file_new_hire_report: nil)
          @employee_uuid = employee_uuid
          @state = state
          @is_work_state = is_work_state
          @questions = questions
          @file_new_hire_report = file_new_hire_report
        end

        sig { params(other: T.untyped).returns(T::Boolean) }
        def ==(other)
          return false unless other.is_a?(self.class)
          return false unless @employee_uuid == other.employee_uuid
          return false unless @state == other.state
          return false unless @is_work_state == other.is_work_state
          return false unless @questions == other.questions
          return false unless @file_new_hire_report == other.file_new_hire_report
          true
        end
      end
    end
  end
end

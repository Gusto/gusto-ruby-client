# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module OpenApiSDK
  module Operations
  
    # An array of earnings for the employee. Depends on your company selections, earnings includes wages, hours, bonuses, tips, commission and more.
    class Earnings < ::OpenApiSDK::Utils::FieldAugmented
      extend T::Sig

      # The amount of the earning.
      field :amount, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('amount') } }
      # The ID of the earning.
      field :earning_id, T.nilable(::Integer), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('earning_id') } }
      # The earning type for the compensation.
      field :earning_type, T.nilable(::OpenApiSDK::Operations::EarningType), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('earning_type'), 'decoder': Utils.enum_from_string(::OpenApiSDK::Operations::EarningType, true) } }
      # The hour of the compensation for the pay period.
      field :hours, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('hours') } }


      sig { params(amount: T.nilable(::String), earning_id: T.nilable(::Integer), earning_type: T.nilable(::OpenApiSDK::Operations::EarningType), hours: T.nilable(::String)).void }
      def initialize(amount: nil, earning_id: nil, earning_type: nil, hours: nil)
        @amount = amount
        @earning_id = earning_id
        @earning_type = earning_type
        @hours = hours
      end
    end
  end
end

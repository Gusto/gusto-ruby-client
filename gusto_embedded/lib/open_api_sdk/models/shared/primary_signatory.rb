# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module OpenApiSDK
  module Shared
  
    # The primary signatory of the company.
    class PrimarySignatory < ::OpenApiSDK::Utils::FieldAugmented
      extend T::Sig

      # The company's primary signatory's email address.
      field :email, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('email') } }
      # The company's primary signatory's first name.
      field :first_name, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('first_name') } }
      # The company's primary signatory's home address.
      field :home_address, T.nilable(::OpenApiSDK::Shared::CompanyHomeAddress), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('home_address') } }
      # The company's primary signatory's last name.
      field :last_name, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('last_name') } }
      # The company's primary signatory's middle initial.
      field :middle_initial, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('middle_initial') } }
      # The company's primary signatory's phone number.
      field :phone, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('phone') } }
      # The UUID of the company's primary signatory.
      field :uuid, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('uuid') } }


      sig { params(email: T.nilable(::String), first_name: T.nilable(::String), home_address: T.nilable(::OpenApiSDK::Shared::CompanyHomeAddress), last_name: T.nilable(::String), middle_initial: T.nilable(::String), phone: T.nilable(::String), uuid: T.nilable(::String)).void }
      def initialize(email: nil, first_name: nil, home_address: nil, last_name: nil, middle_initial: nil, phone: nil, uuid: nil)
        @email = email
        @first_name = first_name
        @home_address = home_address
        @last_name = last_name
        @middle_initial = middle_initial
        @phone = phone
        @uuid = uuid
      end
    end
  end
end

# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module OpenApiSDK
  module Shared
  

    class RequiredAttributes < ::OpenApiSDK::Utils::FieldAugmented
      extend T::Sig

      # A required attribute when creating a garnishment for this state agency. The current values are listed as an enum; though unlikely, values could be added if state agency requirements change in the future.
      field :key, T.nilable(::OpenApiSDK::Shared::Key), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('key'), 'decoder': Utils.enum_from_string(::OpenApiSDK::Shared::Key, true) } }
      # A human readable name of the attribute, e.g. CSE Case Number
      field :label, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('label') } }


      sig { params(key: T.nilable(::OpenApiSDK::Shared::Key), label: T.nilable(::String)).void }
      def initialize(key: nil, label: nil)
        @key = key
        @label = label
      end
    end
  end
end

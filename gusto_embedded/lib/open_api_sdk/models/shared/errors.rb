# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module OpenApiSDK
  module Shared
  

    class Errors < ::OpenApiSDK::Utils::FieldAugmented
      extend T::Sig

      # Specifies the type of error. The category provides error groupings and can be used to build custom error handling in your integration. If category is `nested_errors`, the object will contain a nested `errors` property with entity errors.
      field :category, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('category') } }
      # Specifies where the error occurs. Typically this key identifies the attribute/parameter related to the error.
      field :error_key, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('error_key') } }
      # Provides details about the error - generally this message can be surfaced to an end user.
      field :message, T.nilable(::String), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('message') } }
      # Contains relevant data to identify the resource in question when applicable. For example, to identify an entity `entity_type` and `entity_uuid` will be provided.
      field :metadata, T.nilable(::OpenApiSDK::Shared::EntityErrorObjectMetadata), { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('metadata') } }


      sig { params(category: T.nilable(::String), error_key: T.nilable(::String), message: T.nilable(::String), metadata: T.nilable(::OpenApiSDK::Shared::EntityErrorObjectMetadata)).void }
      def initialize(category: nil, error_key: nil, message: nil, metadata: nil)
        @category = category
        @error_key = error_key
        @message = message
        @metadata = metadata
      end
    end
  end
end

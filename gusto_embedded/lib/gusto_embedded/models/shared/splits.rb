# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module GustoEmbedded
  module Models
    module Shared
    

      class Splits
        extend T::Sig
        include Crystalline::MetadataFields


        field :bank_account_uuid, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('bank_account_uuid') } }

        field :name, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('name') } }
        # An obfuscated version of the account number which can be used for display purposes.
        field :hidden_account_number, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('hidden_account_number') } }

        field :routing_number, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('routing_number') } }
        # The order of priority for each payment split, with priority 1 being the first bank account paid. Priority must be unique and sequential.
        field :priority, Crystalline::Nilable.new(::Integer), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('priority') } }

        field :split_amount, Crystalline::Nilable.new(::Float), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('split_amount') } }

        field :account_type, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('account_type') } }
        # Ciphertext containing the full bank account number, which must be decrypted using a key provided by Gusto. Only visible with the `contractor_payment_methods:read:account_number` scope.
        field :encrypted_account_number, Crystalline::Nilable.new(::String), { 'format_json': { 'letter_case': ::GustoEmbedded::Utils.field_name('encrypted_account_number') } }

        sig { params(bank_account_uuid: T.nilable(::String), name: T.nilable(::String), hidden_account_number: T.nilable(::String), routing_number: T.nilable(::String), priority: T.nilable(::Integer), split_amount: T.nilable(::Float), account_type: T.nilable(::String), encrypted_account_number: T.nilable(::String)).void }
        def initialize(bank_account_uuid: nil, name: nil, hidden_account_number: nil, routing_number: nil, priority: nil, split_amount: nil, account_type: nil, encrypted_account_number: nil)
          @bank_account_uuid = bank_account_uuid
          @name = name
          @hidden_account_number = hidden_account_number
          @routing_number = routing_number
          @priority = priority
          @split_amount = split_amount
          @account_type = account_type
          @encrypted_account_number = encrypted_account_number
        end

        sig { params(other: T.untyped).returns(T::Boolean) }
        def ==(other)
          return false unless other.is_a? self.class
          return false unless @bank_account_uuid == other.bank_account_uuid
          return false unless @name == other.name
          return false unless @hidden_account_number == other.hidden_account_number
          return false unless @routing_number == other.routing_number
          return false unless @priority == other.priority
          return false unless @split_amount == other.split_amount
          return false unless @account_type == other.account_type
          return false unless @encrypted_account_number == other.encrypted_account_number
          true
        end
      end
    end
  end
end
